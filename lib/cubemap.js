...
function Cubemap(pre_url, post_url, container_id, options)
{
	...
	function onmove(e)
	{
		...
		//console.log(that.yaw +": "+ that.pitch);
		that.update();
...
Cubemap.prototype.buildFace = function(face, url)
{
	...
	var fixedHalfSize = halfsize.toFixed(1);

	switch(face)
	{
		case 'front': transform = "translateZ(-"+ fixedHalfSize + "px) rotateY(180deg) rotateX(180deg) rotateZ(180deg)"; break;// VENT
		case 'left': transform = "translateX(" + fixedHalfSize + "px) rotateY(90deg) rotateX(180deg) rotateZ(180deg)"; break; // EMPTY WALL
		case 'right': transform = "translateX(-" + fixedHalfSize + "px) rotateY(-90deg) rotateX(180deg) rotateZ(180deg)";break; // BED WALL
		case 'top': transform = "translateY(-" + fixedHalfSize + "px) rotateX(90deg) rotateY(180deg)";break; // CEILING
		case 'bottom': transform = "translateY(" + fixedHalfSize + "px) rotateX(-90deg) rotateY(180deg)";break; // FLOOR
		case 'back': transform = "translateZ(" + fixedHalfSize + "px) rotateX(180deg) rotateY(0deg) rotateZ(180deg)";break; // BARS
