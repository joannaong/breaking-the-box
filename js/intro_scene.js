(function(){
  ...
  var gamestate = {
    create: function(buttons_elements){
        ...
        var setEndState = function(){
          disableHotSpots();
          parent.window.game.onMiniGameOver();
        }
  ...
  window.scene = {
    ...
    createQuad:function (definition, mouseOverCallback, mouseOutCallback, mouseClickCallback)
    {
      function buildFace (transform) {
        var element = document.createElement('div');
        util.applyTransform(element, transform);
        element.classList.add(definition.style);
        return element;
      }
